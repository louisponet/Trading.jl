...
function fit_γμ(account, ticker1, ticker2, start, stop; timeframe = Minute(1),
                only_day = true)
...
function cointegration_timearray(account, ticker1, ticker2, start, stop; γ = 0.78,
                                 z_thr = 1.5, window = 20, timeframe = Minute(1),
                                 only_day = true)
    ...
    df = merge(df, rename(moving(mean, df[:s], window), :s_mean),
               rename(moving(std, df[:s], window), :s_std))
    ...
    df = merge(df,
               rename(Float64.(sign.(df[:z]) .* sign.(TimeSeries.lag(df[:z], 1)) .== -1),
                      :ExitSignal))

    df = merge(exp.(df[ticksym1]), exp.(df[ticksym2]), df[:z], df[:EntrySignal],
               df[:ExitSignal],
               TimeArray((Time = TimeSeries.timestamp(df),
                          position1 = zeros(length(df[ticksym1])),
                          position2 = zeros(length(df[ticksym2]))); timestamp = :Time),
               df[:s], df[:s_mean], df[:s_std])
    ...
    df = merge(df,
               rename(basecall(diff(df[ticksym1]) .* TimeSeries.lag(df[:position1], 1),
                               cumsum), :gains1))
    df = merge(df,
               rename(basecall(diff(df[ticksym2]) .* TimeSeries.lag(df[:position2], 1),
                               cumsum), :gains2))
...
function Overseer.requested_components(::MomentumPairStrat{horizon}) where {horizon}
    return (Spread, SMA{horizon,Spread}, MovingStdDev{horizon,Spread})
end
...
function plot_simulation(l::AbstractLedger, start = l[TimeStamp][1].t,
                         stop = l[TimeStamp][end].t; kwargs...)
    ...
    purchase_total_values = map(purchase_times) do t
        return values[findmin(x -> abs(x - t), tstamps)[2]]
    end
    sale_total_values = map(sale_times) do t
        return values[findmin(x -> abs(x - t), tstamps)[2]]
    end

    scatter!(p, purchase_times, purchase_total_values; marker = :utriangle, color = :green,
             label = "purchases")
    scatter!(p, sale_times, sale_total_values; marker = :dtriangle, color = :red,
             label = "sales")
...
function plot_indicators(l::AbstractLedger, start = l[TimeStamp][1].t,
                         stop = l[TimeStamp][end].t)
    ...
    ta2 = TimeArray(l)
    plot!(p,
          to(from(merge(ta2[:AAPL_position] ./ 1000, ta2[:MSFT_position] ./ 1000;
                        method = :outer), start), stop))
    return p
end

function pair_trader(broker, ticker1::String, ticker2::String, γ; z_thr = 1.5,
                     momentum = true)
    stratsys = momentum ? [SpreadCalculator(γ), MomentumPairStrat{20}(γ, z_thr)] :
               [SpreadCalculator(γ), PairStrat{20}(γ, z_thr)]
    t = Trader(broker; tickers = [ticker1, ticker2],
               strategies = [Strategy(:pair, stratsys; tickers = ["MSFT", "AAPL"])])
